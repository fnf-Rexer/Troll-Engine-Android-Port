				...
				expr: macro {
					for(fileExt in Paths.HSCRIPT_EXTENSIONS){
						...
						for (filePath in Paths.getFolders(folder))
						{
							...
							//trace(Paths.cleanPath(path));
							if (Paths.exists(Paths.cleanPath(path)))
							{
								//trace('we got it');
								script = funkin.scripts.FunkinHScript.fromFile(Paths.cleanPath(path), Paths.cleanPath(path), _getScriptDefaultVars());
								script.call("new", []);
								break;
							}
						}
							...
							expr.push(macro
								{
									...
									if (script!=null && script.exists($v{name})) {
										script.executeFunc($v{name}, $a{args}, null, [$v{FUNC_EXTENSION_PREFIX + name} => $i{fname}]);
										return;
									}
								}
							);
							...
							expr.push(macro
								{
									...
									if (script != null && script.exists($v{name}))
										return script.executeFunc($v{name}, $a{args}, null, [$v{FUNC_EXTENSION_PREFIX + name} => $i{fname}]);
								}
							);
						}
					...
					case TFun(daArgs, daRet):
						...
						if (returnsVoid){
							expr.push(macro
							{
								...
								if (script!=null && script.exists($v{name})) {
									script.executeFunc($v{name}, $a{args}, null, [$v{FUNC_EXTENSION_PREFIX + name} => $i{superName}]);
									return;
								}
							});
						}else{
							expr.push(macro
							{
								...
								if (script!=null && script.exists($v{name}))
									return script.executeFunc($v{name}, $a{args}, null, [$v{FUNC_EXTENSION_PREFIX + name} => $i{superName}]);
							});
						}
