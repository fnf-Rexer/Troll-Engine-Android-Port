...
{
	public static var touchy:TouchPad;
	public var touchyCam:FlxCamera;
	...
	public function requiresRestart(arr:Array<String>, start:Bool) {
		start = true; 
		for (opt in arr) {
	...
	private function optionSubState() {
		this.persistentDraw = false;
		var daSubstate = new OptionsSubstate();
		daSubstate.isPause = true;
		touchyCam.alpha = 0;
	...
	override public function create()
	{
		...
		touchy = new TouchPad(PlayState.chartingMode ? "LEFT_FULL" : "UP_DOWN", "A");
		add(touchy);

		touchyCam = new FlxCamera();
		touchyCam.bgColor.alpha = 0;
		FlxG.cameras.add(touchyCam, false);
		touchy.cameras = [touchyCam];
	}
	...
	override function destroy() {
		...
		{
			remove(touchy);
			touchy = FlxDestroyUtil.destroy(touchy);
		}

		if(touchyCam != null)
		{
			FlxG.cameras.remove(touchyCam);
			touchyCam = FlxDestroyUtil.destroy(touchyCam);
		}
