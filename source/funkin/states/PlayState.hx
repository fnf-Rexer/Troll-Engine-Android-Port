...
class PlayState extends MusicBeatState
{
	...
	public static var modchartDifficulty:Int = 1;
	...
	public var worldComboOrigin:FlxPoint = new FlxPoint();
	...
	//shitty video
	public var SongVideo:Array<FlxVideoSprite> = [];
	...
	public var EventTween:Array<FlxTween> = [];
	public var EventTimer:Array<FlxTimer> = [];
	...
	override public function create()
	{
		...
		worldComboOrigin.x = 0;
		worldComboOrigin.y = 0;
		...
		*/
		...
		if (hud == null){
			switch(ClientPrefs.etternaHUD){
				...
				case 'V Slice': hud = new DefHUD(boyfriend.healthIcon, dad.healthIcon, SONG.song, stats);
	...
	*/
	...
	override public function update(elapsed:Float)
	{
		...
		*/
		...
		if (!endingSong){
			...
			}else if ((#if android FlxG.android.justReleased.BACK #else touchPad.buttonP.justPressed #end || controls.PAUSE || justUnfocused) && startedCountdown && canPause && !paused) {
				justUnfocused = false;
				openPauseMenu();
			}
		}
	...
	public static function gotoMenus()
	{
		...
		if (isStoryMode){
			...
			funkin.states.FreeplayState.comingFromPlayState = true;
	...
	public var isPixelPrefix:String = "";
	...
	private function displayJudgment(image:String){
		...
		if (worldCombos)
			spr = ratingGroup.displayJudgment(image, worldComboOrigin.x + -40 + comboOffset.x, worldComboOrigin.y + -60 + comboOffset.y);
		else
		...
		if (ClientPrefs.simpleJudge) {
			...
			//if (judgeScale == -1) {
				//spr.scale.set(judgeScale, judgeScale);
			//}
	...
	var judgeScale:Float = -1;
	var comboScale:Float = -1;
	...
	private function displayCombo(?combo:Int){
		...
		if (worldCombos)
			lastCombos = ratingGroup.displayCombo(combo, worldComboOrigin.x + 0 + comboOffset.x, worldComboOrigin.y + 80 + comboOffset.y);
		else
		...
		for (numSpr in lastCombos)
		{
			...
			if (ClientPrefs.simpleJudge)
			{
				...
				//if (comboScale != -1) {
					//numSpr.scale.set(comboScale, comboScale);
				//}
	...
	var comboOffset:FlxPoint = new FlxPoint();
	...
	private function onButtonPress(button:TouchButton):Void {
		...
		if (pressed.contains(buttonCode)) return;
		pressed.push(buttonCode);
		
		if (callOnScripts("onButtonDown", [buttonCode]) == Globals.Function_Stop) // wish this wasnt changed it broke old code of mine lol
			return;

		if (!cpuControlled && startedCountdown && !paused && buttonCode > -1 && button.justPressed) {
			strumKeyDown(buttonCode);
		}
	}
	...
	private function onButtonRelease(button:TouchButton):Void
	{
		...
		pressed.remove(buttonCode);
		
		if (callOnScripts("onButtonUp", [buttonCode]) == Globals.Function_Stop)
			return;
		
		if (!cpuControlled && startedCountdown && !paused && buttonCode > -1) 
		{ 
			strumKeyUp(buttonCode);
		}
	}
...
/* 		for (column => actionBinds in keysArray) {
	...
	public function createLua(path:String):FunkinLua
	{
		var split = path.split("/");
		var modName:String = split[0] == "content" ? split[1] : 'assets';
		
		var script = new FunkinLua(path, [
			"modName" => modName
		]);
	...
	public function openPauseMenu()
	{
		...
		mobile.options.MobileMenu.isPauseMenu = true;
	...
	override function destroy() 
	{
		...
		*/
        ...
        public function playSong(meta:Array<String>, diff:String, diffNum:Int) 
        {
                Song.playSong(new SongMetadata(meta[0], meta[1]), diff, diffNum);
        }

	public function fieldModNumberNote(field:NoteField, modNum:Int) {
		field.modNumber = modNum;
	}

	public function fieldModNumberStrum(field:PlayField, modNum:Int) {
		field.modNumber = modNum;
	}
}
