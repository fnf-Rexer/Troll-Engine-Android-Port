...
#if mobile
import mobile.StorageUtil.StorageType;
#end
...
{
	//android
	var touchStartY:Array<Float> = [-1, -1]; // Initial touch Y position
	var touchEndY:Array<Float> = [-1, -1]; // Current touch Y position
	var scrollVelocity:Array<Float> = [0, 0]; // Stores scroll momentum
	...
	static var options:Map<String, Array<Dynamic>> = [
		...
		#if mobile
		"mobile" => [
			["mobile", ["ScreenSaver", "hitboxPos", "extraButtons", "hitboxType", "controlsAlpha"]],
			[
				"storage",
				[
					"storageType"
	...
	override function create()
	{
		...
		for (idx in 0...optionOrder.length)
		{
			...
			for (data in options.get(tabName))
			{
				...
				if (label == 'storage')	text.text = "storage";
				if (label == 'mobile') text.text = "mobile";
                                text.text = label;
		...
		if (isPause == true) {
			addTouchPad("NONE", "B"); addTouchPadCamera();
		} else {
			addTouchPad("NONE", "B_C"); addTouchPadCamera();
		}
		super.create();
	...
	var daSubstate:mobile.MobileControlSelectSubState;
			...
			case Dropdown:
				...
				if (openedDropdown == widget && overlaps(dropBox, daCamera))
				{
					...
					if (touch != null)
					{
						...
						{
							touchStartY[0] = touch.screenY;
							scrollVelocity[0] = 0;
						}
						else if (touch.pressed)
						{
							touchEndY[0] = touch.screenY;
							var deltaY = touchEndY[0] - touchStartY[0];
						
							if (Math.abs(deltaY) > 5) // Prevent small accidental touches
							{
								camFollow.y -= deltaY * 1.5;
								camFollowPos.y -= deltaY * 1.5;
								touchStartY[0] = touchEndY[0]; // Update touch position for smooth scrolling
							}
						}
						else if (touch.justReleased)
						{
							// Apply momentum effect
							scrollVelocity[0] = (touchEndY[0] - touchStartY[0]) * 0.3;
							FlxTween.num(scrollVelocity[0], 0, 0.5, {ease: FlxEase.quadOut}, (val:Float) -> {
								scrollVelocity[0] = val;
							});
						}
					}
					// Apply touch scroll movement with momentum
					if (Math.abs(scrollVelocity[0]) > 0.1)
					{
						camFollow.y -= scrollVelocity[0];
						camFollowPos.y -= scrollVelocity[0];
						scrollVelocity[0] *= 0.95; // Gradual slowdown
					}
					#end
	...
	override function update(elapsed:Float)
	{
		if (subState == null)
		{
			...
			if (openedDropdown == null)
			{
				...
				if (touch != null)
				{
					if (touch.justPressed)
					{
						touchStartY[1] = touch.screenY;
						scrollVelocity[1] = 0;
					}
					else if (touch.pressed)
					{
						touchEndY[1] = touch.screenY;
						var deltaY = touchEndY[1] - touchStartY[1];
					
						if (Math.abs(deltaY) > 5) // Prevent small accidental touches
						{
							movement -= deltaY * 20 * elapsed; // Match the scrolling effect
							touchStartY[1] = touchEndY[1]; // Update touch position for smooth scrolling
						}
					}
					else if (touch.justReleased)
					{
						// Apply momentum effect
						scrollVelocity[1] = (touchEndY[1] - touchStartY[1]) * 0.3;
						FlxTween.num(scrollVelocity[1], 0, 0.5, {ease: FlxEase.quadOut}, (val:Float) -> {
							scrollVelocity[1] = val;
						});
					}
				}
				// Apply touch scroll movement with momentum
				if (Math.abs(scrollVelocity[1]) > 0.1)
				{
					movement -= scrollVelocity[1] * elapsed;
					scrollVelocity[1] *= 0.95; // Gradual slowdown
				}
				#end					
		...
		if (isPause != true) {
			if (touchPad != null && touchPad.buttonC.justPressed) {
				...
				if (PauseSubState.touchy != null) PauseSubState.touchy.active = PauseSubState.touchy.visible = persistentUpdate = false;
				daSubstate = new MobileControlSelectSubState();
				daSubstate.goBack = (_:Array<String>)->{
		...
		if (subState == null #if android || daSubstate == null #end)
		{
