...
class ChartingState extends MusicBeatState
{
	...
	override function create()
	{
		...
		*/
		...
		/*
		text = (moControls) ? "Up/Down - Change Conductor's strum time
...
/* 		tab_group_song.add(saveMetadata);
	...
	override function update(elapsed:Float)
	{
		...
		if (onGrid){
			...
			if (!moControls) {dummyArrow.x = Math.floor(FlxG.mouse.x / GRID_SIZE) * GRID_SIZE;}
			if (moControls) {for (touch in FlxG.touches.list) dummyArrow.x = Math.floor(touch.x / GRID_SIZE) * GRID_SIZE;}
				
			var gridMult = GRID_SIZE / quantizationMult;
			var rawGridY:Float = 0;
			if (!moControls) {rawGridY = FlxG.mouse.y / gridMult;}
			if (moControls) {for (touch in FlxG.touches.list) rawGridY = touch.y / gridMult;}

			var gridY = Math.floor(rawGridY);

			if (!moControls) dummyArrow.y = (touchPad.buttonY.pressed || FlxG.keys.pressed.SHIFT) ? FlxG.mouse.y : (gridY * gridMult);
			if (moControls) for (touch in FlxG.touches.list) dummyArrow.y = (touchPad.buttonY.pressed || FlxG.keys.pressed.SHIFT) ? touch.y : (gridY * gridMult);

			if (!moControls) {
				if (FlxG.mouse.pressed){
					movedDummyY = (curDummyY != (curDummyY = (touchPad.buttonY.pressed || FlxG.keys.pressed.SHIFT) ? rawGridY : gridY/quantizationMult)); // wtf
		...
		if (!inputBlocked) {
			updateKeys(elapsed);
		}else if (touchPad.buttonA.justPressed || FlxG.keys.justPressed.ENTER) {
	...
	function updateKeys(elapsed:Float) {
		...
		if ((FlxG.keys.pressed.W || FlxG.keys.pressed.S) || (touchPad.buttonUp.pressed || touchPad.buttonDown.pressed))
		{
			...
			else if (touchPad.buttonY.pressed || FlxG.keys.pressed.SHIFT) mult = 4;

			var daTime:Float = 720 * elapsed * mult;

			if (touchPad.buttonDown.pressed || FlxG.keys.pressed.S)
		...
		else if (touchPad.buttonB.justPressed || FlxG.keys.justPressed.BACKSPACE) { 
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null, click:Bool=true):Void
	{
		...
		{
			for (touch in FlxG.touches.list) {
				column = data!=null ? data : Math.floor((touch.x - GRID_SIZE) / GRID_SIZE);
			}			
	    }
		else 
		{
			column = data!=null ? data : Math.floor((FlxG.mouse.x - GRID_SIZE) / GRID_SIZE);	
		}
