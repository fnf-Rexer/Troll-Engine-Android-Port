...
class CharacterEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (testMode) {
			var alt = (touchPad.buttonC.pressed || FlxG.keys.pressed.SHIFT) ? "-alt" : '';
			// who cares anymore
			...
			if (char.animationsArray.length > 0) {
				var replayAnim = ((touchPad.buttonD.justPressed || FlxG.keys.justPressed.S) || (touchPad.buttonV.justPressed || FlxG.keys.justPressed.W) || FlxG.keys.justPressed.SPACE);
				...
				var multiplier:Int = (touchPad.buttonC.pressed || FlxG.keys.pressed.SHIFT) ? 10 : 1;
