...
class ComicsMenuState extends MusicBeatState
{
	...
	{	
		...
		{
			var modDir = mod.directory; trace(modDir);
	...
	static function switchToChapterData(chapterData:ChapterData){
		if (data != null && chapterData != null) switchToChapterNum(data.indexOf(chapterData));
		if (data == null || chapterData == null) {
			if (ComicReader.callBack != null) ComicReader.callBack();
		} 
	}
	...
	override public function create()
	{
		...
		var coverArt = new FlxSprite(0, chapterNameTxt.y + chapterNameTxt.height + 18);
...
class ComicReader extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (font == '') pageNameTitle.font = Paths.font("consola.ttf");
		if (font != '') pageNameTitle.font = Paths.font(font);
		pageNameTitle.alignment = CENTER;
		pageNameTitle.scrollFactor.set(1, 1);
		//if (Fcolor != null) 
		//{
			pageNameTitle.color = Fcolor;
		//}
		add(pageNameTitle);

		var listPage = new TGTTextButton(curPanel.getMidpoint().x, head, 0, "Page List", 18, goToPageList);
		if (bgName == 'corruption') listPage = new TGTTextButton(curPanel.getMidpoint().x, head, 0, "CHAPTER LIST", 18, goToPageList);
		if (font == '') listPage.label.font = Paths.font("consola.ttf");
		if (font != '') listPage.label.font = Paths.font(font);
		...
		//if (Fcolor != null) 
		//{
			listPage.color = Fcolor;
		//}		
		listPage.labelColors[0] = Fcolor;
		listPage.labelColors[2] = Fcolor;
		add(listPage);

		var listPage = new TGTTextButton(curPanel.getMidpoint().x, tail, 0, "Page List", 18, goToPageList);
		if (bgName == 'corruption') listPage = new TGTTextButton(curPanel.getMidpoint().x, tail, 0, "CHAPTER LIST", 18, goToPageList);
		if (font == '') listPage.label.font = Paths.font("consola.ttf");
		if (font != '') listPage.label.font = Paths.font(font);
		listPage.label.underline = true;
		listPage.x -= listPage.frameWidth * 0.5;
		...
		if (pageData.prevPage != null){
			...
			if (bgName == 'corruption') prevPage = new TGTTextButton(curPanel.x + 18, head, 0, "← Next", 18, goPrevPage);
			if (font == '') prevPage.label.font = Paths.font("consola.ttf");
			if (font != '') prevPage.label.font = Paths.font(font);
			prevPage.label.underline = true;
			prevPage.scrollFactor.set(1, 1);
			prevPage.y -= 18;
			//if (Fcolor != null) 
			//{
				prevPage.color = Fcolor;
			//}		
			prevPage.labelColors[0] = Fcolor;
			prevPage.labelColors[2] = Fcolor;
			add(prevPage);

			var prevPage = new TGTTextButton(prevPage.x, tail, 0, "← Page", 18, goPrevPage);
			if (bgName == 'corruption') prevPage = new TGTTextButton(prevPage.x, tail, 0, "← Next", 18, goPrevPage);
			if (font == '') prevPage.label.font = Paths.font("consola.ttf");
			if (font != '') prevPage.label.font = Paths.font(font);
			prevPage.label.underline = true;
			prevPage.scrollFactor.set(1, 1);
		...
		if (pageData.nextPage != null){
			...
			if (bgName == 'corruption') nextPage = new TGTTextButton(0, head, 0, "Next →", 18, goNextPage);
			if (font == '') nextPage.label.font = Paths.font("consola.ttf");
			if (font != '') nextPage.label.font = Paths.font(font);
			...
			//if (Fcolor != null) 
			//{
				nextPage.color = Fcolor;
			//}	
			nextPage.labelColors[0] = Fcolor;
			nextPage.labelColors[2] = Fcolor;
			add(nextPage);

			nextPage = new TGTTextButton(nextPage.x, tail, 0, "Page →", 18, goNextPage);
			if (bgName == 'corruption') nextPage = new TGTTextButton(nextPage.x, tail, 0, "Next →", 18, goNextPage);
			if (font == '') nextPage.label.font = Paths.font("consola.ttf");
			if (font != '') nextPage.label.font = Paths.font(font);
			nextPage.label.underline = true;
			nextPage.scrollFactor.set(1, 1);
    ...
    override function update(elapsed:Float)
    {
        ...
        if (touchPad.buttonLeft.pressed) 
            camFollow.x -= speed;

        if (touchPad.buttonRight.pressed) 
            camFollow.x += speed;
